...
from FourVecEnhancedDataClass import FourVecExtra
...
MTAU = 1.7770505531    # Tau mass in GeV
...
hTauMomentum = ROOT.TH1D("hTauMomentum","; Tau Momentum [GeV]; ",200,0.0,500.0)
hTauPt = ROOT.TH1D("hTauPt","; Tau pT [GeV]; ",200,0.0,200.0)
hTauPDL = ROOT.TH1D("hTauPDL","; Tau proper decay length [um]; ",200,0.0,1000.0)
hTauDL = ROOT.TH1D("hTauDL","; Tau decay length [mm]; ",125,0.0,25.0)
hTauDXY = ROOT.TH1D("hTauDXY","; Tau transverse decay length [mm]; ",125,0.0,25.0)
hTauLeptonIPXY = ROOT.TH1D("hTauLeptonIPXY","; Tau lepton transverse impact parameter [um]; ",200,0.0,1000.0)

hTauElectronIPXY = ROOT.TH1D("hTauElectronIPXY","; Tau electron decay transverse impact parameter [um]; ",200,0.0,1000.0)
hTauMuonIPXY = ROOT.TH1D("hTauMuonIPXY","; Tau muon decay transverse impact parameter [um]; ",200,0.0,1000.0)
hTauAccElectronIPXY = ROOT.TH1D("hTauAccElectronIPXY","; Accepted tau electron decay transverse impact parameter [um]; ",200,0.0,1000.0)
hTauAccMuonIPXY = ROOT.TH1D("hTauAccMuonIPXY","; Accepted tau muon decay transverse impact parameter [um]; ",200,0.0,1000.0)
hTauElectronX = ROOT.TH1D("hTauElectronX","; Tau electron decay El/Etau; ",100,0.0,1.0)
hTauMuonX = ROOT.TH1D("hTauMuonX","; Tau muon decay El/Etau; ",100,0.0,1.0)
hTauAccElectronX = ROOT.TH1D("hTauAccElectronX","; Accepted tau electron decay El/Etau; ",100,0.0,1.0)
hTauAccMuonX = ROOT.TH1D("hTauAccMuonX","; Accepted tau muon decay El/Etau; ",100,0.0,1.0)
...
# Initial state partons     
        ...
        for i in range(nfinal):
            p = (f.readline()).split()
            if int(p[1]) == 2 and (int(p[0]) == 15 or int(p[0]) == -15) and (int(p[2]) != 0):
                tau = FourVecExtra( i+3, int(p[0]), int(p[1]), int(p[2]), float(p[6]), float(p[7]), float(p[8]), float(p[9]) )
                TauParticles.append(tau)
#           print("p ",p)
# Add requirement that the particle is a final state particle
            ...
            if abs(particle.pdgID) == 11:
                ...
                genl = FourVecExtra( i+3, int(p[0]), int(p[1]), int(p[2]), float(p[6]), float(p[7]), float(p[8]), float(p[9]) )
                GenLeptons.append(genl)
...
# Debug taus
        print('Event ',event)
        for tau in TauParticles:
            hTauMomentum.Fill(tau.momentum())
            hTauPt.Fill(tau.pt())
            ctau = -87.03*math.log(rand.Uniform())
            hTauPDL.Fill(ctau)
            hTauDL.Fill(0.001*ctau*tau.momentum()/MTAU)
            dxy = 0.001*ctau*tau.pt()/MTAU       # mm
            hTauDXY.Fill(dxy)
# Neglect bending in magnetic field for now.            
            xv = dxy*tau.px/tau.pt()
            yv = dxy*tau.py/tau.pt()
            tauLeptonEntry = tau.entry
            if event < 10:
                tau.print()
            for glep in GenLeptons:
                 if glep.parent == tauLeptonEntry:
                    print("found match")
                    glep.print()
# Neglect bending in magnetic field for now. Calculate advanced point on lepton trajectory
                    xp = xv + dxy*glep.px/glep.pt()
                    yp = yv + dxy*glep.py/glep.pt()
# So we have two points on the line (xv, yv) and (xp, yp). 
                    slope = (yp - yv)/(xp - xv)
                    intercept = yv - slope*xv
# So eqn is y = slope*x + intercept. Now we want to find the minimum distance 
# to the origin on the line. So we need to minimize x**2 + y**2. 
                    xmin = -slope*intercept/(1.0 + slope**2)
                    ymin = slope*xmin + intercept
                    D = math.sqrt(xmin**2 + ymin**2) 
                    hTauLeptonIPXY.Fill(1000.0*D)
                    if glep.pdgID == 11 or glep.pdgID == -11:
                        hTauElectronIPXY.Fill(1000.0*D)
                        hTauElectronX.Fill(glep.E/tau.E)
                        if glep.accEl(elPtMin):
                            hTauAccElectronIPXY.Fill(1000.0*D)                        
                            hTauAccElectronX.Fill(glep.E/tau.E)                        
                    else:
                        hTauMuonIPXY.Fill(1000.0*D)
                        hTauMuonX.Fill(glep.E/tau.E)
                        if glep.accMu(muPtMin):
                            hTauAccMuonIPXY.Fill(1000.0*D)                        
                            hTauAccMuonX.Fill(glep.E/tau.E) 
        if len(leptons) == 2:
